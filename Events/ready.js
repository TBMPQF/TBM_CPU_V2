...
async function scheduleNextGame(guildId) {
  ...
  if (bingoTimer && new Date(bingoTimer.nextBingoTime) > new Date()) return;
...
async function startBingoGame(guildId) {
  ...
  if (bingoTimer && new Date(bingoTimer.nextBingoTime) > new Date()) {
    ...
    const delayToNextBingo = randomInterval(2, 5);
    nextBingoTime = new Date(Date.now() + delayToNextBingo);
    await BingoTimer.findOneAndUpdate(
      { serverID: guildId },
      { nextBingoTime },
  ...
  const delayInMillis = nextBingoTime.getTime() - Date.now();
  setTimeout(async () => {
    ...
    const bingoCollector = bingoChannel.createMessageCollector({
      time: 300000, // 5 minutes (300000ms) pour trouver le bon nombre
    });
