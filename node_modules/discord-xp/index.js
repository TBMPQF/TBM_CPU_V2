...
class DiscordXp {
  ...
  * @param {string} [userId] - Discord user id.
  * @param {string} [guildId] - Discord guild id.
  * @param {number} [message] - Amount of message to append.
  */

   static async appendSMS(userId, guildId, message) {
    if (!userId) throw new TypeError("An user id was not provided.");
    if (!guildId) throw new TypeError("A guild id was not provided.");
    if (!message) throw new TypeError("An amount of message was not provided.");

    const user = await levels.findOne({ userID: userId, guildID: guildId });
    if (!user) return false;
    
    user.message += parseInt(message, 10);
 
    user.save().catch(e => console.log(`Erreur lors d'ajout d'un SMS: ${e}`) );
